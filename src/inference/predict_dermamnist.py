...
Single-label classification: one predicted class per image.

Usage:
    from src.inference.predict_dermamnist import predict_derma
    result = predict_derma(image_tensor)   # (3, H, W) RGB tensor
"""
...
def predict_derma(image_tensor: torch.Tensor,
    ...
    Returns:
        dict with keys:
            "class_id"     : int — predicted class index (0–6)
